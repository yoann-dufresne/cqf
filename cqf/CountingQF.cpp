...
uint64_t CountingQF::getRem(uint8_t * blockAddr, uint32_t slot)
{
    ...
    res <<= MEM_UNIT;
    cout << bitset<8>(*slotAddr) << endl;

    uint32_t iterations = ((remainderLen + (MEM_UNIT - 1)) / MEM_UNIT);
...
uint64_t CountingQF::getRemRev(uint8_t * blockAddr, uint32_t slot)
{
    uint32_t slotOffset = (slot * remainderLen) / MEM_UNIT;
    uint32_t bitOffset = (slot * remainderLen) % MEM_UNIT;
    uint64_t res = 0;
    int32_t iterations = ((remainderLen + (MEM_UNIT - 1)) / MEM_UNIT) - 1;
    uint8_t remainingBits = remainderLen - (MEM_UNIT * (iterations));

    uint8_t * slotAddr = blockAddr + 17 + slotOffset + iterations;
    uint8_t firstBitMask = ((1ULL << (MEM_UNIT - remainingBits)) - 1);

//    cout << "first bit mask: " << bitset<8>(firstBitMask) << endl;
//    cout << "first slot: " << bitset<8>(*slotAddr) << endl;
/*
    cout << "first slot: "<< bitset<8>(*slotAddr) << endl;
    cout << "first mask: "<< bitset<8>(firstBitMask) << endl;
    cout << "empty res: " << bitset<64>(res) << endl;
*/
    res += (*slotAddr & firstBitMask);

//    cout << "slot added to res and shifted: " << bitset<64>(res) << endl;
    
    for (uint8_t i = 1; i < iterations; i++)
    {
        slotAddr -= 1;

//        cout << "next slot: "<< bitset<8>(*slotAddr) << endl;
        res <<= MEM_UNIT;
        res += *slotAddr; 
//        cout << "slot added to res and shifted: " << bitset<64>(res) << endl;
    }
...
//    cout << "last slot: " << bitset<8>(*slotAddr) << endl;

    res <<= MEM_UNIT;
    uint8_t lastBitMask = (((1ULL << (MEM_UNIT - bitOffset))) - 1);

    //cout << "last bit mask: " << bitset<8>(lastBitMask) << endl;
...
void CountingQF::setRemRev(uint8_t * blockAddr, uint32_t slot, uint64_t rem)
{
    ...
    */
    ...
    // Set the first part of the remainder
    cout << "first slot: "<< bitset<8>(rem & 0xFF) << endl;
