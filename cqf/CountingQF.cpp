...
void CountingQF::setRem(uint8_t * blockAddr, uint32_t slot, uint64_t rem)
{
    ...
    cout << "remainder: " << bitset<64>((rem >> (remainderLen - MEM_UNIT))) << endl;
    
    // BlockAddr + Offset + Occupieds and Runends length + slotOffset
    uint8_t * slotAddr = blockAddr + 1 + ((MAX_UINT / MEM_UNIT) * 2) 
        + slotOffset;

    cout << "first part of remainder" << bitset<64>((rem >> (MAX_UINT - MEM_UNIT))) << endl;
    ...
    {
        cout << "shfited by how much:" << (MAX_UINT - MEM_UNIT) << endl;
